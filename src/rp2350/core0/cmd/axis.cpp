...
//
// The only command file that writes the position model; the rest of the
// writers are in ops/ (the axis-map commit, homing, probe). Origin
// invalidation once scattered across one 1000-line file bred "updated one
// frame, forgot the other" bugs, so every write goes through position.h's
// named operations.
...
// Open a probe session. See core0/ops/probe.h for why this is a session and not a
