...
def bezier_deriv1(c, t):
    ...
    mt = 1 - t
    return (
        3*(mt*mt*(c.p1[0]-c.p0[0]) + 2*mt*t*(c.p2[0]-c.p1[0]) + t*t*(c.p3[0]-c.p2[0])),
        3*(mt*mt*(c.p1[1]-c.p0[1]) + 2*mt*t*(c.p2[1]-c.p1[1]) + t*t*(c.p3[1]-c.p2[1])),
    )
...
def arc_length(c):
    ...
    for t, w in zip(_GL5_NODES, _GL5_WEIGHTS):
        d = bezier_deriv1(c, t)
        total += w * math.sqrt(d[0]*d[0] + d[1]*d[1])
    return total
...
def curvature(c, t):
    ...
    speed = math.sqrt(d1[0]*d1[0] + d1[1]*d1[1])
    if speed < 1e-10:
        return 0.0
